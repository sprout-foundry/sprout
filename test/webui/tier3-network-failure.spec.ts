...
//   - `chat-input` — chat input textarea (NOT in registry; shared with tier2-steer-input)
//   - `chat-send` — send button (NOT in registry; shared with tier2-steer-input)
//   - `disconnected-overlay` — the DisconnectedOverlay component (NOT in registry)
...
test.describe('Network Failure During Chat', () => {
  test.fixme(
    'chat works normally before simulating failure (control test) — chat-input testid missing (SP-087-6 followup)',
    async () => {
  ...
  test.fixme(
    'chat shows error state when API endpoint is blocked — chat-input testid missing (SP-087-6 followup)',
    async () => {
      await page.goto(vite.url, { waitUntil: 'networkidle' });
      await expect(page.getByTestId(TESTIDS['chat-shell'])).toBeVisible({ timeout: 30_000 });
  ...
  test.fixme(
    'chat shell remains stable after WebSocket disconnect — disconnected-overlay testid missing (SP-087-6 followup)',
    async () => {
      await page.goto(vite.url, { waitUntil: 'networkidle' });
      await expect(page.getByTestId(TESTIDS['chat-shell'])).toBeVisible({ timeout: 30_000 });
      ...
      // NOTE: disconnected-overlay is NOT in the testid registry.
      const disconnectedOverlay = page.locator('[class*="disconnected-overlay"]').first();
  ...
  test.fixme(
    'chat recovers when network is restored after failure — chat-input testid missing (SP-087-6 followup)',
    async () => {
      await page.goto(vite.url, { waitUntil: 'networkidle' });
      await expect(page.getByTestId(TESTIDS['chat-shell'])).toBeVisible({ timeout: 30_000 });
