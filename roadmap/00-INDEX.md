...
| SP-137 | [Vision & Image Analysis: First-Class Paths, Provider Neutrality, Native OCR](./SP-137-vision-first-class.md) | 🟡 Draft (2026-09-08) — not started. Fixes: seed registry discarding tool-result images (PDF multimodal dead), `HasVisionCapability` hardcoded provider allowlist (zai-coding et al. excluded), `read_file` binary garbage on images, dead multimodal analyze handlers, no prompt discoverability. Excises Ollama-coupled OCR config (`PDFOCR*` → neutral `ocr_fallback_model`); adds native OCR tier (macOS Vision / Windows OCR / tesseract). Rule: no provider names in vision-tier code. |
