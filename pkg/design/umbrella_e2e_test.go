...
func TestUmbrella_EveryArtifactOpensInNonSproutTool(t *testing.T) {
	...
	umbTokens, err := ResolveExportTokens(root)
	require.NoError(t, err)
	for _, target := range ExportTargets {
		...
		// The banner is a comment line on the comment-syntax targets and a
		// leading field on the JSON target; the shared extractor reads both.
		require.Equalf(t, umbTokens.InputHash, provenanceSourceHash(content),
