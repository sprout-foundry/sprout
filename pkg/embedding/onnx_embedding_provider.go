...
//
// ORT's session.Run ignores Go contexts — a Run that hangs (degenerate input
// shape, intra-op thread pool contention, etc.) keeps spinning until it
// finishes or the process dies. On slow devices a hung Run holds the
...
func (p *ONNXEmbeddingProvider) runWithOptions(
	...
	if err != nil {
		...
		return p.session.Run(inputs, outputs)
	}
	defer opts.Destroy()

	// Watchdog: on ctx cancellation, signal ORT to terminate the Run.
	// done is closed when runWithOptions returns so the watchdog knows to
	// exit without racing the deferred opts.Destroy().
	...
	if err := p.session.RunWithOptions(inputs, outputs, opts); err != nil {
