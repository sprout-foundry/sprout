...
| SP-127 | [Promote Filesystem Gate to Gate-1](./SP-127-filesystem-gate-promotion.md) | 🟢 M1–M3 Shipped (`a06a3f8a` M1, `1acff1cb`–`ac969831` M2, `<HEAD>` M3). Path-tier classification unified into Gate 1; all 6 file-touching handlers migrated to `PrecheckFileAccess`; audit trail, docs, conformance tests, and integration tests all updated. M4 (remove old machinery) deferred pending benchmark. |
