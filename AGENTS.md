...
CI (`.github/workflows/build.yml`) runs on push to `main` and PRs. The "Build and Test" workflow matrix-tests on ubuntu/macOS/Windows, then runs a WASM smoke test and builds 6 binaries (3 OSes × 2 arches). The chain was red for hours in one session due to seven distinct root causes — the notes below capture the lessons so future agents don't repeat them.

### Reproducing CI locally

The fast gates (run in seconds):
```bash
make vet          # go vet ./...
make fmt-check    # gofmt -l across pkg/ cmd/ internal/
make lint         # eslint + prettier + tsc (webui)
make build-all    # React UI + WASM + Go binary
```
The slow gate (10–25 min with race detection):
```bash
make test-coverage                  # full suite, ~60% coverage, 40% threshold
SKIP_NETWORK_TESTS=1 make test-coverage   # skip ZAI/network integration tests locally
```
Smoke tests and WASM roster:
```bash
make test-smoke
make prepare-grammars && bash scripts/wasm-tool-roster-smoke.sh
```

### `//go:embed` directories that are gitignored

`pkg/webui/static/` and `pkg/ast/grammars/bin/` are both gitignored (generated at build time) but embedded via `//go:embed`. A fresh CI checkout has neither. **The embed directive fails if no matching files exist** — this breaks `go vet` (which runs BEFORE `make build-all` populates the dirs).

- `pkg/webui/static/placeholder` is a tracked file (via a `.gitignore` `!` exception) that keeps `//go:embed static` satisfied on clean checkouts. If you add a new gitignored embed dir, add a tracked placeholder the same way.
- `pkg/ast/grammars/bin/*.bin` is regenerated by `make prepare-grammars`. Any CI job that cross-compiles (WASM smoke, build matrix) must run `make prepare-grammars` first. The `Makefile` targets do this automatically via the `prepare-grammars` prerequisite; standalone scripts/workflow steps do not.

### Coverage tooling crashes on packages with no test files

`go test -coverprofile` invokes `go tool covdata`, which crashes when processing a package that has zero test files for the target platform:
- Windows: `STATUS_DLL_INIT_FAILED` (0xC000013A) — unrecoverable OS-level process kill.
- Some Linux toolchains: `go: no such tool "covdata"`.

The `test-coverage` Makefile target filters these out via `go list -f '{{.TestGoFiles}}'`, which correctly handles build-tag-constrained files (e.g. `console_windows_test.go` is excluded on non-Windows, so `pkg/utils/console` is treated as no-test on Linux/macOS). **When adding a new package with no tests, no action is needed** — the filter handles it automatically.

### Go 1.25 test-runner crash on Windows

Go 1.25's test runner crashes with `STATUS_DLL_INIT_FAILED` on Windows **after all tests pass** — it's in the test-binary teardown, not any test. The crash kills the parent bash process, so no shell-level error handling can intercept it. This is handled two ways in `build.yml` / `Makefile`:
- `TEST_COVER=no` on Windows: skips `-coverprofile` (coverage is collected on ubuntu/macOS instead).
- `continue-on-error: ${{ runner.os == 'Windows' }}` on the test step: the crash exit code is non-fatal on Windows only. Ubuntu/macOS remain strict.
If a future Go version fixes this, revert both — search for `STATUS_DLL_INIT_FAILED`.

### Test isolation for git commands

Tests that execute git commands through the agent shell handler (`handleShellCommand`) must prevent git from launching an interactive editor:
- `git rebase -i`, `git commit`, `git merge` all invoke `$GIT_EDITOR` (default: `vi`/`vim`), which **blocks forever on stdin** in non-interactive `go test` environments.
- The `newIsolatedTestAgent` helper in `pkg/agent/agent_creation_test.go` sets `GIT_EDITOR=true`, `GIT_SEQUENCE_EDITOR=true`, `GIT_MERGE_AUTOEDIT=no` via `t.Setenv`. **All agent tests should use this helper**, not `agent.NewAgent()` directly — the latter reads real/global config and is non-hermetic.

### Non-hermetic tests (the #1 source of "passes locally, fails in CI" and vice-versa)

A test that calls `agent.NewAgent()` (not the temp-config helper) reads the developer's real `~/.config/sprout` config, or `SPROUT_CONFIG` if set. This causes:
- Persona state leaks (`DisabledPersonas` populated from a prior run's `manage_settings` calls).
- Network tests that hit real APIs (`TestZAIProvider*` fails with HTTP 429 when `ZAI_API_KEY` has no balance — these skip in CI because `build.yml` doesn't set the key).

**Always use `createTestAgentWithTempConfig(t)` or `newTestAgent(t)`**, which isolate `HOME`, `SPROUT_CONFIG`, and API keys to temp dirs.

### Map-iteration flakiness

Go randomizes map iteration order. A test that picks "the first entry from a map" is flaky by construction — it gets a different entry every run. When a test needs a specific map entry, **filter to one that satisfies the test's preconditions** (e.g. an enabled persona, a non-nil value) rather than taking `range`'s first yield.

### Data races: `go:embed`/background-goroutine field access

When a struct has both a background goroutine (e.g. `warmModelsCache`) and setter methods (e.g. `SetHTTPClient`), every field the goroutine reads must be guarded by the same mutex the setters use. The pattern: **snapshot under `RLock` → release → use the local** (never hold the lock during I/O like `.Do()`). See `pkg/agent_providers/generic_provider.go` for the canonical implementation.
