...
- [ ] SP-082-2: Update `patch_structured_file` to read the original file as a `*yaml.Node` and apply patches via an adapter (`yamlNodeFromMap` / `mapFromYamlNode`). _Status: **DEFERRED** — depends on SP-082-1 (and the deeper PHASE 3 refactor of applyPatchOperation to walk yaml.Node directly)._
