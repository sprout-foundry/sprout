...
- [x] SP-002-mistralToolCalls-parse: `RecoverMistralToolCalls` in `pkg/agent_api/mistral_toolcalls.go` parses both shapes (`[TOOL_CALLS]<name>{json}` repeated, and `[TOOL_CALLS][{"name":…,"arguments":…}]`), strips the marker, and synthesizes IDs. Wired into `pkg/agent_api/unified.go` `SendChatRequest` + `SendVisionRequest`, gated on tools having been offered and no structured tool calls present.
- [x] SP-002-mistralToolCalls-stream: Same recovery applied at the end of `UnifiedProviderWrapper.SendChatRequestStream` (the accumulated streamed response flows through the unified wrapper before reaching callers).
## Not Started — Hydrate Test Fixture: Large-Payload Streaming

- [ ] webui-coldHydrate-largePayloadFixture: `TestHandleColdHydrateRequest_EstimateSeconds`'s `~2MB` and `~4MB` subtests are currently skipped — the in-process WebSocket test pair (`newTestingConnPair` in `pkg/webui/cold_hydrate_test.go`) cannot reliably stream payloads >1MB. The handler writes succeed up to ~1MB, then the WS connection fails mid-stream and the read helper used to panic with "repeated read on failed websocket connection" (now returns early on close, but the run never reaches `hydrate_complete`). Replace the fixture with one that handles large buffered writes (e.g. a real `net.Pipe` paired with `gorilla/websocket` over an `httptest.Server`), then remove the `t.Skip` in the subtest and verify the ETA assertions for the 2MB / 4MB cases.

- [x] SP-002-mistralToolCalls-test: `pkg/agent_api/mistral_toolcalls_test.go` covers both shapes, double-encoded args, multiple calls, leading prose, and negative cases. Live-validated: `mistralai/ministral-8b-2512` now passes the probe's complex stage (was failing with "no todos submitted").
