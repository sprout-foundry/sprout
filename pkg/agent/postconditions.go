...
func verifyPythonStructure(code string, names []string) (bool, string) {
	...
	for i := 0; i < len(lines); i++ {
		if m := defRe.FindStringSubmatch(lines[i]); len(m) > 1 {
			if blockEndsProperly(lines, i) {
				found[m[1]] = true
			}
		}
		if m := clsRe.FindStringSubmatch(lines[i]); len(m) > 1 {
			if blockEndsProperly(lines, i) {
				found[m[1]] = true
			}
		}
	...
	for _, n := range names {
		...
		if _, ok := found[n]; !ok && len(names) > 0 {
			return false, fmt.Sprintf("python structure check: missing complete block for %s", n)
		}
	}
...
func verifyJSStructure(code string, names []string) (bool, string) {
	...
	if !bracesBalanced(code) {
		return false, "js structure check: unbalanced braces"
	}
	...
	for _, n := range names {
		if _, ok := found[n]; !ok && len(names) > 0 {
			return false, fmt.Sprintf("js/ts structure check: missing definition for %s", n)
		}
	}
...
func verifyRubyStructure(code string, names []string) (bool, string) {
	...
	if depth != 0 {
		return false, "ruby structure check: unmatched end"
	}
...
func verifyPHPStructure(code string, names []string) (bool, string) {
	...
	if !bracesBalanced(code) {
		return false, "php structure check: unbalanced braces"
	}
	fRe := regexp.MustCompile(`(?m)^\s*function\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(`)
	cRe := regexp.MustCompile(`(?m)^\s*class\s+([A-Za-z_][A-Za-z0-9_]*)`)
	found := map[string]bool{}
	for _, m := range fRe.FindAllStringSubmatch(code, -1) {
		if len(m) > 1 {
			found[m[1]] = true
		}
	}
	for _, m := range cRe.FindAllStringSubmatch(code, -1) {
		if len(m) > 1 {
			found[m[1]] = true
		}
	}
	for _, n := range names {
		if _, ok := found[n]; !ok && len(names) > 0 {
			return false, fmt.Sprintf("php structure check: missing definition for %s", n)
		}
	}
...
func verifyRustStructure(code string, names []string) (bool, string) {
	if !bracesBalanced(code) {
		return false, "rust structure check: unbalanced braces"
	}
	...
	for _, n := range names {
		if _, ok := found[n]; !ok && len(names) > 0 {
			return false, fmt.Sprintf("rust structure check: missing fn %s", n)
		}
	}
