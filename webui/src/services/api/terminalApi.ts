...
export async function getAvailableShells(fetchFn: typeof fetch): Promise<any> {
  const response = await fetchFn('/api/terminal/shells');
  if (!response.ok) throw new Error('Failed to fetch available shells');
  return response.json();
}

export async function getTerminalHistory(fetchFn: typeof fetch, sessionId?: string): Promise<any> {
...
export async function addTerminalHistory(fetchFn: typeof fetch, command: string): Promise<any> {
