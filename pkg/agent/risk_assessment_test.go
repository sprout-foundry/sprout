...
// ---------------------------------------------------------------------------
// Shadow-mode parity — full corpus (≥100 unique commands)
// ---------------------------------------------------------------------------

func TestShadowMode_ParityFullCorpus(t *testing.T) {
	// One agent instance for the entire corpus — keeps the test fast.
	agent := newTestAgent(t)
	defer agent.Shutdown()
	...
	// Helper: decision rank for "one-notch-stricter" checks.
	decisionRank := func(d string) int {
		switch d {
		case "allow":
			return 0
		case "prompt":
	...
	// ---- Category 1: Safe reads / queries ----
	// Expected: both paths say "allow" (or new is one notch stricter → "prompt").
	safeReads := []string{
		"ls -la",
		"ls",
		"cat README.md",
	...
	// ---- Category 3: Caution-tier (builds, installs, file ops) ----
	// Expected: both paths say "prompt" (or new is one notch stricter).
	cautionTier := []string{
		"npm install",
		"npm run build",
		"go build ./...",
	...
	// ---- Category 4: Git write + history operations ----
	// These are NOT hard-blocked by the old classifier. Old path says "prompt"
	// or "allow". New path can be "prompt" or (for history-rewrites with
	// AllowGitHistoryRewrite=false) "block" due to the git-history-rewrite gate.
	highGit := []string{
	...
	// ---- Category 5: Dangerous / high-risk operations (not hard-block) ----
	// Old path says "prompt". New path must be at least "prompt" (can be "block").
	dangerousOps := []string{
		"rm -rf build/",
		"rm -rf dist/",
		"rm -rf target/",
	...
	// ---- Category 6: Critical / hard-block operations ----
	// BOTH paths must say "block". Only includes commands that are actually
	// hard-blocked (IsHardBlock=true) by the classifier — not merely
	// ShouldBlock=true without IsHardBlock (those are known divergences
	// between the old ShouldBlock gate and the new Level-based gate).
	criticalOps := []string{
		"rm -rf /",
		"rm -rf /*",
		"rm -rf ~",
	...
	// Verify corpus size ≥ 100.
	total := len(safeReads) + len(safeGitReads) + len(cautionTier) +
		len(highGit) + len(dangerousOps) + len(criticalOps)
	if total < 100 {
		t.Fatalf("corpus has %d commands, need at least 100", total)
	}

	// ---- Run each category as a sub-test ----

	// Category 1: safe reads — new can match or be one notch stricter
	t.Run("safe_reads", func(t *testing.T) {
		for _, cmd := range safeReads {
			t.Run(cmd, func(t *testing.T) {
				args := map[string]interface{}{"command": cmd}
				secResult := tools.ClassifyToolCall("shell_command", args)
				oldDecision := resolveOldDecision(secResult)
				...
				oldRank := decisionRank(oldDecision)
				newRank := decisionRank(newDecision)

				if newRank < oldRank {
					t.Errorf("new decision %q is less restrictive than old %q — %s",
						newDecision, oldDecision, newAssessment.Explain())
				} else if newRank-oldRank > 1 {
					t.Errorf("new decision %q is more than one notch stricter than old %q — %s",
						newDecision, oldDecision, newAssessment.Explain())
				}
			})
		}
	...
	// Category 2: safe git reads — new can match or be one notch stricter
	t.Run("git_reads", func(t *testing.T) {
		for _, cmd := range safeGitReads {
			t.Run(cmd, func(t *testing.T) {
				args := map[string]interface{}{"command": cmd}
				secResult := tools.ClassifyToolCall("shell_command", args)
				oldDecision := resolveOldDecision(secResult)
	...
	// Category 3: caution-tier — new can match or be one notch stricter
	t.Run("caution_tier", func(t *testing.T) {
		for _, cmd := range cautionTier {
			t.Run(cmd, func(t *testing.T) {
				args := map[string]interface{}{"command": cmd}
				secResult := tools.ClassifyToolCall("shell_command", args)
				oldDecision := resolveOldDecision(secResult)
	...
	// Category 4: git write/history — new can be "prompt" or "block"
	// (history-rewrite commands are blocked by AllowGitHistoryRewrite gate).
	t.Run("git_write_history", func(t *testing.T) {
		for _, cmd := range highGit {
			t.Run(cmd, func(t *testing.T) {
				args := map[string]interface{}{"command": cmd}
				secResult := tools.ClassifyToolCall("shell_command", args)
				oldDecision := resolveOldDecision(secResult)
				...
				// Old path for these commands is "allow" or "prompt".
				// New path must not be LESS restrictive than old.
				oldRank := decisionRank(oldDecision)
				newRank := decisionRank(newDecision)
				...
				// New path CAN be stricter (history-rewrite gate can push to "block").
				// No upper bound assertion — the new path is allowed to be stricter.
			})
		}
	})

	// Category 5: dangerous ops — new must not be less restrictive than old
	t.Run("dangerous_ops", func(t *testing.T) {
		for _, cmd := range dangerousOps {
			t.Run(cmd, func(t *testing.T) {
				args := map[string]interface{}{"command": cmd}
				secResult := tools.ClassifyToolCall("shell_command", args)
				oldDecision := resolveOldDecision(secResult)
				...
				// New path CAN be stricter (e.g., "prompt" → "block").
				// No upper bound assertion — the new path is allowed to be stricter.
			})
		}
	})

	// Category 6: critical — BOTH paths must say "block"
	t.Run("critical_ops", func(t *testing.T) {
		for _, cmd := range criticalOps {
			t.Run(cmd, func(t *testing.T) {
				args := map[string]interface{}{"command": cmd}
				secResult := tools.ClassifyToolCall("shell_command", args)
				oldDecision := resolveOldDecision(secResult)
