...
export function useTabManagement({
  ...
  const closeBuffer = useCallback((bufferId: string) => {
    ...
    setBuffers(prev => {
      ...
      if (buffer.paneId && nextPaneBuffer) {
        ...
        if (r) next.set(nextPaneBuffer.id, { ...r, isActive: currentActivePane === buffer.paneId, paneId: buffer.paneId });
      }
      return next;
    });
    if (buffer.paneId)
      setPanes(prev => prev.map(p => p.id === buffer.paneId ? { ...p, bufferId: nextPaneBuffer?.id || null } : p));
    if (bufferId === activeBufferId) setActiveBufferId(nextPaneBuffer?.id || null);
  }, [activeBufferId, activePaneIdRef, buffersRef, isAutoSaveEnabled, saveBuffer, setBuffers, setPanes, setActiveBufferId]);

  const closeAllBuffers = useCallback(() => {
    const cb = buffersRef.current;
    const closableIds = Array.from(cb.entries()).filter(([_, b]) => b.isClosable !== false).map(([id]) => id);
    for (const bid of closableIds) {
      ...
      setBuffers(prev => { const n = new Map(prev); n.delete(bid); return n; });
    }
    ...
    setPanes(prev => prev.map(p => ({
      ...p, bufferId: closableIds.includes(p.bufferId || '') ? (nextBuffer?.id || null) : p.bufferId,
    })));
  ...
  const closeOtherBuffers = useCallback((keepBufferId: string) => {
    const cb = buffersRef.current;
    const ids = Array.from(cb.entries()).filter(([id, b]) => id !== keepBufferId && b.isClosable !== false).map(([id]) => id);
  ...
  return {
    activateBuffer, switchToBuffer, closeBuffer, closeAllBuffers,
    closeOtherBuffers, reorderBuffers, moveBufferToPane,
  };
}
