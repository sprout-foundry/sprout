...
**Counts (as of 2026-08-19):** 104 shipped · 12 pending · 2 parked in `future/`.
...
root and the historical record is the git log. Status in this table can
lag the codebase — verify against `git log --grep=<spec>` before
treating an entry as unstarted._

| Spec | Title | Status |
|------|-------|--------|
| SP-138 | [Crash-Safe Session Persistence & Interrupted-Turn Recovery](./SP-138-crash-safe-session-recovery.md) | ✅ Shipped (2026-08-18, `v0.17.17`) — All 5 phases: atomic session writes + `.bak`; per-session turn journal (WAL) with per-iteration snapshots via OnIteration; load-time journal replay + dangling-tool-call repair + interrupted-state marking; interrupted-session badges + recovery loads in CLI picker and WebUI; force-quit paths save before exit. Final review's MUST_FIX (per-iteration journaling) and all SHOULD_FIXes landed in `632bd6090`. |
...
| SP-133 | [Config / State / Cache / Secret Separation](./SP-133-config-state-separation.md) | 🟢 Shipped (2026-08-04, `82b6f0931`; embeddings-under-data-root follow-up `7fc7954b8`) — splits config/state/data+cache/credentials into four category roots (`pkg/envutil`: ConfigDir/StateDir/DataDir/CacheDir), `config.local.json` user-scope layer, `CredentialsDir()` 0700, `credentials/` excluded from support bundles, one-shot migration with `migrated_sp133` marker (`pkg/configuration/migration_sp133.go`), CI guard `TestNoLegacyHomeDotSprout`, and `sprout config get --show-origin`. `~/.sprout` legacy layout removed. |
| SP-134 | [GPU Acceleration for Native macOS Embeddings (CoreML EP)](./SP-134-gpu-macos-embeddings.md) | 🟢 Shipped via MLX, superseding the CoreML-EP route (2026-08-07, `ab379d5be` + review follow-ups `97eebbf75`/`8be061adc`; merged `3721d4116`) — `MLXEmbeddingProvider` (`pkg/embedding/mlx_provider.go`, darwin/arm64 + cgo + mlx build tags) loads Jina Code v2 safetensors directly in fp16 on Metal, with `SPROUT_EMBEDDING_BACKEND` override (`mlx`/`cpu`), inferenceGate concurrency, and automatic ONNX-CPU fallback. The specced CoreML-EP re-export path was not pursued; MLX replaced it. |
| SP-135 | [Code-Specific Embedding Model (smaller than Gemma, ~100k repos)](./SP-135-code-embedding-model.md) | 🟢 Adopt path shipped (2026-08-07) — Jina Code v2 (137M, BERT encoder trained on The Stack v2) is the code embedding model for retrieval + duplicate detection (`pkg/embedding/jina_provider.go`, `JinaCodeV2Config()`), replacing Gemma in the index path; Gemma remains for conversation/NL semantics. ONNX int8 CPU export + MLX fp16 Metal variant both wired. The fine-tuning phase of the spec was not pursued. |
| SP-136 | [Daemon-First Architecture — Shared Process Model](./SP-136-daemon-first-architecture.md) | 🟢 Shipped — all 5 phases complete (2026-08-07, P0–P4; `b24c44f6a` P0 file-lock index writes, P1 multi-workspace hardening, P2 lazy daemon auto-start, `8861b82a4` P3 embedding service via Unix socket, `9569836f6` P4 full CLI-on-daemon). CLI routes embedding + one-shot agent work through the daemon with in-process fallback and `SPROUT_DAEMON=0` / `SPROUT_DAEMON_AGENT=0` escape hatches. See also the sibling spec `SP-136-cross-platform-local-llm.md` (separate workstream: tensor.Backend + GGML; op stubs and cross-platform hardware verification remain open there). |
