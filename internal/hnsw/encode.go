...
func (g *SavedGraph[K]) Save() error {
	tmp, err := newPendingFile(g.Path)
	if err != nil {
		return err
	}
	defer tmp.cleanup()

	wr := bufio.NewWriter(tmp.f)
	err = g.Export(wr)
	if err != nil {
...
// pendingFile is a tiny replacement for google/renameio's PendingFile that
// works on Windows. Upstream coder/hnsw v0.6.1 imports
// github.com/google/renameio (v1), whose TempFile is gated by a
// `// +build !windows` constraint, so any Windows build of hnsw fails to
// compile. This vendored copy of hnsw swaps that one call site for the
// helper below: write to a sibling temp file in the same directory, fsync,
// then os.Rename onto the target. Rename is atomic on POSIX and atomic-
// per-file on Windows (NTFS) for our access pattern (no concurrent
// readers holding open handles to the target between rename calls).
...
func newPendingFile(target string) (*pendingFile, error) {
	...
	// final rename stays on the same filesystem (a cross-filesystem
	// rename would fail with EXDEV on Linux).
	...
	return &pendingFile{f: f, target: target}, nil
}
...
func (p *pendingFile) closeAtomicallyReplace() error {
	...
	if err := os.Rename(p.f.Name(), p.target); err != nil {
		_ = os.Remove(p.f.Name())
		return err
	}
	p.done = true
