/**
 * WebSocket event handler hook.
 *
 * Extracted from useWebSocketEvents to separate concerns.
 * This hook handles all incoming WebSocket events and updates
...
export interface UseEventHandlerOptions {
  ...
  lastConnectionStateRef: MutableRefObject<boolean>;
...
export function useEventHandler({
  ...
  queuedMessagesRef,
  setQueuedMessages,
}: UseEventHandlerOptions): UseEventHandlerReturn {
        ...
        if (newConnectionState !== lastConnectionStateRef.current) {
          ...
          connectionTimeoutRef.current = setTimeout(() => {
            ...
            setState((prev) => ({
              ...
              sessionId: prev.sessionId || (incomingSessionId ?? null),
