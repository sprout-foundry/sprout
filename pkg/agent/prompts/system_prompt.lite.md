...
- **Images & PDFs**: images arrive inline by default (pastes, `read_file`, fetched image URLs); `[image N: … described via provider/model]` provenance means a textual description replaced the pixels — re-run `analyze_image_content` for more fidelity. Modes: `ocr`, `general`. Never dump binary, never improvise external OCR scripts
