...
- [ ] SP-082-3: Add round-trip tests in `pkg/agent_tools/write_structured_handler_test.go` and `pkg/agent_tools/patch_structured_handler_test.go` — JSON, YAML, and `package.json`-style ordering. _Status: **DEFERRED** — depends on SP-082-1 and SP-082-2._
- [ ] SP-082-4: Update the tool definitions to note that key order is preserved. Acceptance: a `package.json` written via the tool retains the LLM-supplied key order; a 1-field patch produces a 1-line diff; `go test ./pkg/agent_tools/...` green; existing patch tests still pass. _Status: **DEFERRED** — depends on SP-082-1 and SP-082-2._
