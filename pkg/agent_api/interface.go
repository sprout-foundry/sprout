...
// SproutLocalClientType is the on-device LLM provider. It runs a local
// Go MLX server (cmd/llm_server) on Apple Silicon, managed automatically
// by the agent — no API key needed, no manual server start.
...
func GetProviderName(clientType ClientType) string {
	...
	// sprout-local displays as "Local (Offline)"
	if clientType == SproutLocalClientType {
