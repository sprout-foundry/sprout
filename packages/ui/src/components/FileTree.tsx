...
export interface FileTreeProps {
  ...
  selectedFile?: string;
  /** Root path for the file tree (default: '.') */
  rootPath?: string;
  /** Callback when the tree needs refreshing */
  onRefresh?: () => void;
  /** Callback after an item is created */
  onItemCreated?: () => void;
  /** Callback after an item is deleted */
    ...
    return (
      <div className="file-tree" data-testid="file-tree">
        ...
        {contextMenu !== null && !multiSelectDisabled && multiSelect.selectedPaths.size >= 2 && multiActions.isSelected(contextMenu.file.path) ? (
          <ContextMenu isOpen={contextMenu !== null} x={contextMenu.x} y={contextMenu.y} onClose={() => setContextMenu(null)}>
            <button className="context-menu-item danger" onClick={handleBatchDelete} disabled={multiSelect.isBatchBusy}>
              <Trash2 size={14} />
              <span>Delete ({multiSelect.selectedPaths.size} items)</span>
            </button>
            <button className="context-menu-item" onClick={handleBatchMove} disabled={multiSelect.isBatchBusy}>
              <FolderInput size={14} />
              <span>Move to…</span>
            </button>
            <div className="context-menu-divider" />
            <button className="context-menu-item" onClick={() => handleCopyPaths(false)}>
              <ClipboardCopy size={14} />
              <span>Copy relative paths</span>
            </button>
            {workspaceRoot && (
              <button className="context-menu-item" onClick={() => handleCopyPaths(true)}>
                <ClipboardCopy size={14} />
