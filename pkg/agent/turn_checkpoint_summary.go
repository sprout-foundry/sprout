...
// (in the deleted conversation_optimizer_summary.go). The optimizer dependency
// was incidental — they only needed message-walking and content-shape
// detection — so they moved to free functions when the optimizer became a
// seed-backed wrapper.

// maxSummaryEntries bounds the number of bullet points retained in a
// turn-checkpoint summary; matches the value that used to live in
// PruningConfig.Structural.MaxSummaryEntries.
...
// they can be echoed in the wrapped summary header. Both are normalized to
// fit a single bullet line.
