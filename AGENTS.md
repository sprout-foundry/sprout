...
CI (`.github/workflows/build.yml`) matrix-tests on ubuntu/macOS/Windows, then runs a WASM smoke test and builds 6 binaries (3 OSes × 2 arches). The rules below prevent the most common breakages.

### Always run the gates before pushing

```bash
make vet && make fmt-check && make lint && make build-all   # fast gates (~30s)
SKIP_NETWORK_TESTS=1 make test-coverage                      # slow gate (~15min)
make prepare-grammars && bash scripts/wasm-tool-roster-smoke.sh   # WASM build
```

If any gate fails locally, it will fail in CI. Fix before pushing.

### Every `//go:embed` of a gitignored directory needs a tracked placeholder

`pkg/webui/static/` and `pkg/ast/grammars/bin/` are generated at build time and gitignored. `//go:embed` fails on a fresh checkout if no files match the pattern. Two rules:

1. **Add a tracked placeholder file** via a `.gitignore` `!` exception (see `pkg/webui/static/placeholder`). This is what keeps `go vet` green on clean checkouts — it runs before `make build-all` populates the dir.
2. **Run `make prepare-grammars` in any CI step** that compiles standalone (not through a Make target). The WASM smoke job, build matrix, and any script that does `go build` directly must prepare grammar blobs first. Make targets include it automatically; standalone scripts and workflow steps do not.

### Write hermetic tests

Every test must be self-contained — no dependency on the developer's real config, env, or global state. Required practices:

- **Use `newTestAgent(t)` / `createTestAgentWithTempConfig(t)`**, never `agent.NewAgent()` directly. The helpers isolate `HOME`, `SPROUT_CONFIG`, and API keys to temp dirs. `NewAgent()` reads real config and leaks state across tests.
- **Scope all env with `t.Setenv`** — never mutate process-global env in a test body.
- **Guard network tests** behind an env-var skip (`SKIP_NETWORK_TESTS` or a credential like `ZAI_API_KEY`). CI runners have no API keys; the test must skip cleanly when the key is absent.
- **Prevent interactive-editor hangs**: tests that execute git commands inherit `GIT_EDITOR=true` from `newIsolatedTestAgent`. If you write a new test helper, set `GIT_EDITOR`, `GIT_SEQUENCE_EDITOR`, and `GIT_MERGE_AUTOEDIT` — otherwise `git rebase -i` / `git commit` block forever on stdin.

### Don't pick "first map entry" — it's flaky

Go randomizes map iteration order. A test that takes the first entry from a `range` loop over a map gets a different value every run. When you need a map entry, **filter to one that satisfies the test's preconditions** (e.g. an enabled persona, a non-nil value) rather than taking the first yield.

### Guard every field a background goroutine reads

When a struct spawns a background goroutine (e.g. `warmModelsCache`) AND has setter methods (e.g. `SetHTTPClient`), every field the goroutine reads must go through the same mutex the setters use. The pattern:

```go
p.mu.RLock()
local := p.field      // snapshot under read-lock
p.mu.RUnlock()
// use `local` for all downstream work — NEVER hold the lock during I/O (.Do(), etc.)
```

Writers take the exclusive lock. See `pkg/agent_providers/generic_provider.go` for the canonical implementation.

### Known platform workarounds (revisit when Go updates)

The `test-coverage` Makefile target and `build.yml` carry two Windows-specific accommodations for Go 1.25 bugs. Search for `STATUS_DLL_INIT_FAILED` if you need context:

1. **No-test packages** are filtered out of `go test -coverprofile` via `go list -f '{{.TestGoFiles}}'` — `go tool covdata` crashes on packages with zero test files for the target platform.
2. **Windows test step uses `continue-on-error`** + `TEST_COVER=no` — Go 1.25's test runner crashes during teardown after all tests pass. Ubuntu/macOS remain strict.
...
- **Local history ops** (`git checkout`, `restore`, `reset`, `stash`) are restorable via ChangeTracker (`list_changes`, `recover_file`, `revert_my_changes`) and may run via `shell_command`. Prefer scoped ops over whole-tree resets; use `recover_file` for agent edits (touches only files the agent changed).
- **NEVER REBASE** in any form — `git rebase` (interactive or otherwise), `git pull --rebase`, `git rebase --continue`/`--skip`. The only permitted rebase invocation is `git rebase --abort`, used solely to escape a half-finished rebase left by a prior session before resuming the merge workflow. Rebase rewrites shared history; the resulting "ahead/behind" divergence forces `--force` to push, which destroys other contributors' work. **Use `git merge` to integrate upstream** — the "Pre-Push Safety Check" below documents the canonical pattern (`fetch` → `log HEAD..FETCH_HEAD` → `merge FETCH_HEAD` → `build` → commit → push). If you find yourself mid-rebase from a prior session, `git rebase --abort` to recover the pre-rebase `main`, then `git merge origin/main` to integrate.
...
**The classifier is behavior-based with a default-SAFE fallback.** It detects risky *patterns* (rm, sudo, kill, eval, pipe-to-shell, redirection, state changes, etc.) and returns `SecuritySafe` for any command that matches none of them. This replaces the original fragile architecture — a hardcoded name whitelist where unknown commands defaulted to CAUTION.

**Do NOT:**
- **Reintroduce a name-based whitelist or default-CAUTION fallback.** That was the original architecture; it required manual registration of every read-only tool (`nvidia-smi`, `rocm-smi`, etc.) to avoid false-positive prompts. The legacy `safeListCommands` map still exists for commands that need prefix-matching logic, but it should shrink over time, not grow.
- **Attempt an embedding-based classifier.** Tried and removed — embeddings conflate `rm -rf node_modules` and `rm -rf /etc` (nearly identical vectors, opposite risk). A tokenizing command parser is the correct tool.
- **Add command names to `safeListCommands` to fix false positives.** Instead, add the risky *behavior* pattern to `isCautionPattern` / `isDangerousPattern` / `configuration.IsCriticalOperation`.

**When adding coverage for a new risky behavior:** add the pattern to `isCautionPattern` (recoverable risk, prompts user) or `isDangerousPattern` (irreversible damage) in `shell_patterns.go`, the matching risk-type string to `getShellCommandRiskType` in `shell_utils.go`, and test cases to `security_test.go`.
...
Conversation context is managed by **three distinct operations**, plus an orthogonal embedding store. Conflating them is the recurring failure mode — the design depends on each operation staying in its lane. The full design history lives in git log (search `SP-066`); the shipped code is `pkg/agent/checkpoint_compaction*.go`, `pkg/agent/turn_embedding.go`, and `pkg/agent/rollup*.go`.
