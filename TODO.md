...
[x] - BACKEND: Remove global env mutation in config manager — `NewManagerWithLayers()` calls `envutil.SetEnv("CONFIG", dir)` which mutates process-level environment variables. Pass paths explicitly through function arguments instead. `pkg/configuration/config.go`
...
[x] - SP-011 Phase 1: Add terminal search via @xterm/addon-search — Ctrl+Shift+F opens search bar above terminal. Text input with next/prev, match counter, case/regex toggles. Close on Escape. `webui/src/components/TerminalSearchBar.tsx`
[x] - SP-011 Phase 1: Add clickable file paths in terminal — Register link provider via `Terminal.registerLinkProvider()` detecting patterns like `./foo.go:12:34`, `foo.go:12`. On click, dispatch event to open file in editor at correct line/col. `webui/src/extensions/terminalFilePaths.ts`
...
[x] - REFACTOR: Split `websocket.go` (1,334 lines) into: `safe_conn.go` (SafeConn wrapper), `websocket_handler.go` (main handler), `terminal_websocket.go` (terminal WS), `websocket_message_handlers.go` (message routing), `websocket_security_handlers.go` (approval/provider change). `pkg/webui/websocket.go`
...
[x] - SECURITY: Add content size limits to `edit.go` and `write.go` — `read.go` caps at 32KB/2MB (reduced from 80KB/10MB per SP-024) but edit/write have no limits; LLM could write megabytes in a single operation. `pkg/agent_tools/edit.go`, `pkg/agent_tools/write.go`
[x] - CONTEXT: Lower `read_file` default limit from 80KB to 32KB (~8K tokens) — 80KB consumes ~15% of a 128K context window; 32KB aligns with the ~500-line sweet spot used by Cursor and other tools. See SP-024. `pkg/agent_tools/read.go`
[] - CONTEXT: Lower `read_file` default limit from 80KB to 32KB (~8K tokens) — 80KB consumes ~15% of a 128K context window; 32KB aligns with the ~500-line sweet spot used by Cursor and other tools. See SP-024. `pkg/agent_tools/read.go`
[x] - CONTEXT: Lower `read_file` line-range limit from 10MB to 2MB — 10MB allows accidental ~2.5M token reads; 2MB is a more reasonable safety valve. See SP-024. `pkg/agent_tools/read.go`
[] - CONTEXT: Add `repo_map` tool — lightweight AST-based codebase overview (~1K tokens) showing file paths, function signatures, and type declarations so agents understand structure before reading files. Follows Aider's approach. See SP-024.
[x] - CONTEXT: Add observation masking — replace old tool outputs with placeholders after processing to prevent quadratic context accumulation. JetBrains research shows 52% cheaper and 2.6% more effective than summarization. See SP-024.
