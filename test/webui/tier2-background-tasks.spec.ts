...
test.describe('Background Tasks', () => {
  test('Background status visible after launching long-running command', async () => {
    ...
    const popover = page.getByTestId(TESTIDS['background-tasks-popover']);
  ...
  test('resume/terminate control visible on a background task', async () => {
    ...
    const taskItems = page.getByTestId(TESTIDS['background-task-item']);
    const taskCount = await taskItems.count();

    if (taskCount > 0) {
      const firstTask = taskItems.first();
      const attachBtn = firstTask.locator('[data-testid="background-task-attach"]');
      const killBtn = firstTask.locator('[data-testid="background-task-kill"]');
