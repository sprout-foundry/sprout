...
| SP-127 | [Promote Filesystem Gate to Gate-1](./SP-127-filesystem-gate-promotion.md) | 🟡 Phase 2 shipped (`e5dc181e` 2.2 schema+loader, `389a9d31` 2.1 cd gate, `1e1e4bb9` 2.3 step-aware apply/restore, `0a0a93e3` 2.4+2.5 resolver scope + symlink re-validate, `3396f0c3` 2.6 audit events, `dc167b03` 2.6 LogJSON type-drop fix). M1–M4 gate consolidation still open. Workflow authors can now declare `allowed_paths` at workflow, initial, and step level; the shell `cd` gate enforces the allowlist at the shell layer; the resolver re-validates symlink targets at write time; and every allowed/denied/redirected decision writes a JSONL audit entry. | |
