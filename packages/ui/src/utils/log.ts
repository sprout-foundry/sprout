...
const getProductionLevel = () => {
  return typeof process !== 'undefined' && process.env?.NODE_ENV === 'production' ? Levels.warn : Levels.debug;
};
...
export function debugLog(...args: unknown[]) {
  ...
  if ((typeof process === 'undefined' || process.env?.NODE_ENV !== 'production') && minLevel <= Levels.debug) {
    console.log(...args);
  }
}
