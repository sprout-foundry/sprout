...
def generate_html_summary_report(conversion_results, report_dir):
    ...
    html_content.append("        .checkbox-container { margin-top: 10px; }")
    html_content.append("        .checkbox-container label { font-weight: normal; }")
    html_content.append("        .checkbox-container input[type='checkbox'] { margin-right: 8px; }")
    html_content.append("    </style>")
    html_content.append("</head>")
    html_content.append("<body>")
    ...
    def add_file_section_html(title, file_list, status_class):
        ...
        if file_list:
            ...
            for item in file_list:
                ...
                # Add checkbox for manual review
                if "manual check required" in item['status'].lower() or "no ledit output" in item['status'].lower() or "parse error" in item['status'].lower():
                    section_parts.append(f"                <div class='checkbox-container'>")
                    section_parts.append(f"                    <label><input type='checkbox' id='checkbox-{report_relative_path}' onclick='toggleReviewStatus(this, \"{report_relative_path}\")'> Mark as reviewed</label>")
    ...
    html_content.append("        }")
    html_content.append("")
    html_content.append("        function toggleReviewStatus(checkbox, reportPath) {")
    html_content.append("            // This function would handle saving the review status")
    html_content.append("            // For now, it just provides visual feedback")
    html_content.append("            const fileItem = checkbox.closest('.file-item');")
    html_content.append("            if (checkbox.checked) {")
    html_content.append("                fileItem.style.backgroundColor = '#e6ffe6';")
    html_content.append("                fileItem.style.borderLeft = '4px solid green';")
    html_content.append("            } else {")
    html_content.append("                fileItem.style.backgroundColor = '';")
    html_content.append("                fileItem.style.borderLeft = '';")
    html_content.append("            }")
    html_content.append("        }")
    html_content.append("    </script>")
