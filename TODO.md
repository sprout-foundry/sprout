...
- [ ] **SP-090-16:** Run `npm run test:webui-e2e` locally against the full stack (`sprout serve --mock-llm` + Vite). Fix any flaky or genuinely-broken tests discovered now that they execute. Acceptance: the full suite (70 tests across 27 files) is green or has documented `test.skip()` with a root-cause note for each skip.
- [ ] **SP-090-17:** Confirm `.github/workflows/webui-e2e.yml` runs clean across all 4 shards. Acceptance: zero `test.fixme()` in `test/webui/`; `grep -rn 'test.fixme' test/webui/` returns nothing; CI is green; `make build-all` clean.
- [ ] **SP-090-18:** Delete the now-obsolete `test/webui/testids-gap-report.md` and `test/webui/tier3-gap-report.md` (the gaps they document are closed). Update `roadmap/SP-087-acceptance.md` criterion 3 from FAIL to PASS.
