...
func TestValidateSyntax_ReturnsNonNilOnStdinStatError(t *testing.T) {
	...
	// In Go 1.25+, gofmt always errors on stdin. Assert we get that error.
	if err == nil {
		t.Fatalf("expected non-nil error from gofmt stdin stat (Go 1.25+ behavior); got nil")
	}
	t.Logf("gofmt returned error as expected: %v", err)
}
