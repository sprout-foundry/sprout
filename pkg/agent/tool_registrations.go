...
func newDefaultToolRegistry() *ToolRegistry {
	...
	registry.RegisterTool(ToolConfig{
		Name:        "run_subagent",
		Description: "Delegate ONE focused task to a subagent. Use for sequential / dependent work, or when you need to review output before continuing. For 2+ independent tasks use `run_parallel_subagents`.\n\n**`persona` is REQUIRED** — pick one (e.g. general, coder, debugger, tester, reviewer, researcher, web_scraper). Each persona gets a curated tool subset for determinism.\n\nNo timeout. Provider/model come from `subagent_provider` / `subagent_model` config.\n\n**Result contract**: response is a JSON envelope; `files_modified` (also mirrored as `[subagent files modified] … [/subagent files modified]` at the top of `stdout`) is the AUTHORITATIVE list of files this subagent edited. Trust it: if a file is not in that list, the subagent did not change it. Do NOT revert, undo, or treat as out-of-scope any file in the working tree unless you've independently confirmed it's unrelated to this subagent's reported changes AND unrelated to your own prior edits this session. When in doubt, ask the user before reverting.",
		Parameters: []ParameterConfig{
