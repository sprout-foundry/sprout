...
| SP-143 | [Screen Runtime — Interactive Screens with Tokens and Flows](./SP-143-screen-runtime.md) | 📋 Draft (2026-09-25) — one shared, provenance-hashed `design/runtime/sprout-screens.js` + `generated/tokens.css` referenced by every screen (never authored per screen); runtime intercepts `data-nav` anchors for in-place multiscreen flow previews; webui srcDoc preview rewrites workspace-relative refs to `/api/file` so references resolve in the tool. Item 1.1 (preview ref rewriting) independently shippable. Not started. |
