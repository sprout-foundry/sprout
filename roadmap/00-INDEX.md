...
| SP-140 | [Vision as a First-Class Input](./SP-140-vision-capability-first-class.md) | 🟡 Phase 1 landed (2026-09-18) — `api.ResolveVisionCapability` single resolver (probe → declared precedence); `SupportsConversationalVision` deleted from both interfaces and all ~20 implementations, and the conversational-vision distinction itself removed (OCR-only models flow inline; single `AcceptsImages` field); `effectiveVisionSupport`/`effectiveConversationalVision` + agent probe cache deleted; batch splitters merged into `pkg/agent_tools`. Net −870 LoC. Phases 2–4 (optimistic inline + verify, delegation tier, WebUI parity) open. |
