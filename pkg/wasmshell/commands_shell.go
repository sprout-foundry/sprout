...
func cmdHelp(args []string, stdin string) CmdResult {
	...
	out.WriteString("  git <read-only sub> status, diff, log, show, branch, remote, ls-files,\n")
	out.WriteString("                     rev-list, rev-parse (browser shell allowlist)\n")
	out.WriteString("\nShell features:\n")
	out.WriteString("  |   Pipe commands\n")
	out.WriteString("  &&  Run next only if previous succeeded\n")
	out.WriteString("  ||  Run next only if previous failed\n")
	out.WriteString("  ;   Run sequentially\n")
	out.WriteString("  >   Redirect stdout to file\n")
	out.WriteString("  >>  Append stdout to file\n")
	out.WriteString("  2>  Redirect stderr (2>/dev/null discards)\n")
