 ...
 */
export function useBufferMutations(setBuffers: Dispatch<SetStateAction<Map<string, EditorBuffer>>>) {
  const updateBufferContent = useCallback(
    (bufferId: string, content: string) => {
      setBuffers((prev) => {
  ...
  const updateBufferCursor = useCallback(
    (bufferId: string, position: { line: number; column: number }) => {
      setBuffers((prev) => {
        const next = new Map(prev);
        const buffer = next.get(bufferId);
        if (buffer) {
  ...
  const updateBufferScroll = useCallback(
    (bufferId: string, position: { top: number; left: number }) => {
      setBuffers((prev) => {
        const next = new Map(prev);
        const buffer = next.get(bufferId);
        if (buffer) {
  ...
  const setBufferModified = useCallback(
    (bufferId: string, isModified: boolean) => {
      setBuffers((prev) => {
        const next = new Map(prev);
        const buffer = next.get(bufferId);
        if (buffer) {
  ...
  const setBufferOriginalContent = useCallback(
    (bufferId: string, originalContent: string) => {
      setBuffers((prev) => {
        const next = new Map(prev);
        const buffer = next.get(bufferId);
        if (buffer) {
  ...
  const setBufferLanguageOverride = useCallback(
    (bufferId: string, languageId: string | null) => {
      setBuffers((prev) => {
        const next = new Map(prev);
        const buffer = next.get(bufferId);
        if (buffer) {
  ...
  const revertBufferToOriginal = useCallback(
    (bufferId: string) => {
      setBuffers((prev) => {
        const next = new Map(prev);
  ...
  const setBufferExternallyModified = useCallback(
    (bufferId: string, diskContent: string, mtime?: number) => {
      setBuffers((prev) => {
        const next = new Map(prev);
        const buffer = next.get(bufferId);
        if (buffer) {
  ...
  const clearBufferExternallyModified = useCallback(
    (bufferId: string) => {
      setBuffers((prev) => {
        const next = new Map(prev);
        const buffer = next.get(bufferId);
