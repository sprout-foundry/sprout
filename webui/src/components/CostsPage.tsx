...
export default function CostsPage({ onSessionClick, onBack }: CostsPageProps = {}) {
  ...
  return (
    <div className="costs-page" data-testid="costs-page">
      ...
      {!loading && !error && hasData && (
        <>
          {/* Summary band: all-time total + billing-type pills on one row.
              (The four period cards below cover Today/Week/Month/Total —
              this band only adds the billing-type breakdown.) */}
          <div className="costs-summary-band" data-testid="costs-summary-total">
            <div className="costs-summary-band-total">
              <span className="costs-summary-band-label">All-time total</span>
              <span className="costs-summary-band-value">${summary!.total_cost.toFixed(4)}</span>
