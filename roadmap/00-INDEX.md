...
| SP-140 (vision) | [Vision as a First-Class Input](./SP-140-vision-capability-first-class.md) | ✅ Phases 1–4 core landed (2026-09-18) — resolver (runtime > probe > declared), runtime capability learning with text-only reretry, delegation tier (`DelegateImageDescriptions` + structured-description prompt + provenance labels) on the inline chat path, URL content sniffing for extension-less image URLs, assumed-vision prompt posture, WebUI upload e2e; per-model `vision_limits` landed 2026-09-22 (catalog projection was already in place). Remaining: designer-flow batch refinements (deferred to designer spec). |
| SP-141 | [pkg/agent Package Decomposition](./SP-141-agent-package-decomposition.md) | 📋 Proposed (2026-09-19) — plan-only. `pkg/agent` reached 238 non-test files / ~51K LOC in one package; spec proposes incremental extraction into six subpackages (workflow, changes, approvals, subagents, tools) behind an agentapi seam, one phase per focused session, no behavior changes. Not yet scheduled. |
| SP-142 | [Chat Mode Lanes — Per-Mode Chat Ownership on the Server](./SP-142-chat-mode-lanes.md) | 🟢 Server stamping + client lane filtering shipped 2026-09-24 (`d0941b207` carried the §8.2 workbench, `2b70888ec` landed 142.1/.2; `3ae4c1821` fixed the daemon shared-mode regression). 142.3–142.5 (workspace query gate + queue UX + design header affordances) open. |
...
| SP-143 | [Screen Kit — Interactive, Token-Driven Screens](./SP-143-screen-runtime.md) | ✅ Shipped (2026-09-25, 143.1–143.7: 02d0a7b7c, 6efb132d5, 61e647897, 3a2741dbf, 1bd095b98, 505bdb973, 8684472bf, f0e1927f9) — export utility layer + json target; scaffold assets (chrome.css, base templates, runtime); preview ref rewriting through /api/file; screens.json derived index + validator rules; generator surface; e2e proving chrome/theming/in-place nav/states/zero-write restyle (design_kit.spec.ts, 4 tests). Known follow-up: /api/file sends no cache headers (masked a regen in e2e until cache-disable); REPLACE_WITH_STEM scaffolding pin. |

| SP-140 (design) | [Design Workspace — Agent-Native UX Design on Open Formats](./SP-140-design-workspace.md) | ✅ SP-140-1…7 + 10 shipped (2026-09-22, merged via `fe93ae98a`, released in v0.18.12); SP-140-8/9 drafted, not started. Umbrella spec: directory contract, phase map, open-format charter; premises: co-evolution (no handoff) and git as design's source control. Coordinates SP-140-1…10. |
| SP-140-1 | [Design Format Charter — DTCG Tokens, SVG Wireframes, Mermaid Flows](./SP-140-1-formats.md) | ✅ Shipped (2026-09-22, merged via `fe93ae98a`). |
| SP-140-2 | [Designer Persona, Design Skill, and Agent Design Tools](./SP-140-2-persona-tools.md) | ✅ Shipped (2026-09-22, merged via `fe93ae98a`). Depends on SP-140-1. |
| SP-140-3 | [WebUI DesignView — Flow Canvas and Screen Browser](./SP-140-3-designview.md) | ✅ Shipped (2026-09-22, merged via `fe93ae98a`). Depends on SP-140-1. |
| SP-140-4 | [Visual Loop — Render→Critique, Consistency Checks, Human Feedback](./SP-140-4-visual-loop.md) | ✅ Shipped (2026-09-22, merged via `fe93ae98a`). Depends on SP-140-2, SP-140-3. |
| SP-140-5 | [Design↔Code Sync — Continuous Bidirectional Loop](./SP-140-5-sync.md) | ✅ Shipped (2026-09-22, merged via `fe93ae98a`). Export + `design_sync` (code→design import), directional drift reporting. Depends on SP-140-1 (core); SP-140-4 for full value. |
| SP-140-6 | [The Loop Surface — Agent State in the Design Space](./SP-140-6-loop-surface.md) | ✅ Shipped (2026-09-22, `849f41282` / PR #85, merged via `fe93ae98a`) — live inventory, `/api/design/status` + health strip, annotation pins, agent panel in Design mode, critique findings sidecar. |
| SP-140-7 | [Human Co-Editing — Revision-Checked Writes, Conflict UX](./SP-140-7-co-editing.md) | ✅ Shipped (2026-09-22, items 7.1–7.6 landed, status flipped in `c54e04ce2`, merged via `fe93ae98a`). |
| SP-140-8 | [Screen-Centric IA — Screen Workbench as the Shell's Primary Axis](./SP-140-8-screen-centric-ia.md) | 🟡 In progress (2026-09-24) — 8.1 + 8.2 (screen workbench, `d0941b207`) landed; 8.3 (adaptive chrome) remains. |
| SP-140-9 | [Format Rework — HTML-First Screens, Derived Flows](./SP-140-9-format-rework.md) | ✅ Shipped (2026-09-26, 9.1–9.8): a305b83a2 (§9a identity + deprecation), 2c93979cf (flow sources + derived .mmd + drift), 00a235cf4 (flows export target + render pins), ad0d131e7..a9f2cdb07 (the 9.4 migration: 11 wireframes → screens, 4 flows → .json), 1a2761374 (tier removal, rules walk screens). Wireframes/ is gone (presence = error); flows are .json sources with derived hash-guarded .mmd; dogfood tree validates 0 findings. | 
| SP-140-10 | [Design-Mode Chat UX — Placement, Pinning, Visibility, Inline Tool Details](./SP-140-10-chat-ux.md) | ✅ Shipped (2026-09-22, items 10a–10d + 10e dogfood record `5f29d4e6c`, merged via `fe93ae98a`; manual dogfood items 1–4 verified in a 2026-09-22 local-build browser pass). Four dogfood gripes: RTL-aware side column, per-mode conversation pinning, full-height + live visibility, inline tool details retiring the sidebar dependency (shared chat mechanism; Code-mode regression gates). |
