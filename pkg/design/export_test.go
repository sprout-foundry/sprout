...
func TestRenderCSSVariables(t *testing.T) {
	...
	// distinct line openers is exact.)
	for _, opener := range []string{"/* Generated by design_export_tokens", "/* source-hash: ", "/* Source: design/tokens/"} {
		assert.Equal(t, 1, countOccurrences(out, opener), "one %q banner line", opener)
	}
	assert.Equal(t, 3, countOccurrences(out, " */\n"), "every banner line closes its comment")
	// The provenance header carries the token-input content hash (item 5.2).
...
// (block-comment) targets.
var exportProvenanceHashRE = regexp.MustCompile(`source-hash: (fnv1a64:[0-9a-f]{16})`)
...
func TestRenderArtifacts_EveryArtifactHeaderAndHash(t *testing.T) {
	...
	for _, a := range artifacts {
		...
		assert.Contains(t, string(a.Content), "source-hash: "+tokens.InputHash,
			"%s must carry the token-input hash", a.RelPath)
