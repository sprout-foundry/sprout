...
const RepoDetailPage: React.FC<RepoDetailPageProps> = ({ repoOwner, repoName, onBack }) => {
  ...
  const fetchDetail = useCallback(async () => {
    ...
    const branches = branchesRes.status === 'fulfilled' ? (await branchesRes.value.json().catch(() => [])).map((b: { name?: string }) => b.name || String(b)) : [];
    const commits = commitsRes.status === 'fulfilled' ? (await commitsRes.value.json().catch(() => [])).map((c: { sha?: string; commit?: { message?: string; author?: { name?: string; date?: string } } }) => ({
      sha: c.sha?.slice(0, 7) ?? '',
      message: c.commit?.message?.split('\n')[0] ?? '',
      author: c.commit?.author?.name ?? '',
      date: c.commit?.author?.date ?? '',
    })) : [];
    const pulls = pullsRes.status === 'fulfilled' ? (await pullsRes.value.json().catch(() => [])).map((p: { number?: number; title?: string; state?: string }) => ({
      number: p.number ?? 0,
      title: p.title ?? '',
      state: p.state ?? 'open',
    })) : [];
  ...
  if (error || !data) {
    return (
      <div className="platform-page">
        <button className="btn btn-sm btn-ghost" onClick={onBack}><ArrowLeft size={14} /> Back</button>
        <div className="platform-card"><p className="platform-empty-text">{error || 'Repo not found'}</p></div>
      </div>
    );
  }
  ...
  return (
    <div className="platform-page">
      <button className="btn btn-sm btn-ghost" onClick={onBack}><ArrowLeft size={14} /> Back to Repos</button>
      ...
      <div className="platform-card repo-detail-header">
        ...
        <div className="repo-detail-actions">
          <a className="btn btn-sm" href={`/webui/?repo=${encodeURIComponent(data.repo.html_url)}`}>Browser IDE</a>
      ...
      <div className="dashboard-grid">
        ...
        <div className="platform-card">
          <div className="platform-card-header"><h3><GitCommit size={16} /> Recent Commits</h3></div>
        ...
        {/* Pull requests */}
        <div className="platform-card">
          <div className="platform-card-header"><h3><GitPullRequest size={16} /> Pull Requests</h3></div>
      ...
      {data.branches.length > 0 && (
        <div className="platform-card">
          <div className="platform-card-header"><h3><GitBranch size={16} /> Branches ({data.branches.length})</h3></div>
          <div className="branch-chips">
            {data.branches.slice(0, 20).map(b => (
              <span key={b} className="branch-chip">{b}</span>
            ))}
          </div>
        </div>
      )}
