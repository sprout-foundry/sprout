...
| SP-140 | [Vision as a First-Class Input](./SP-140-vision-capability-first-class.md) | 🟡 Phases 1–2 landed (2026-09-18) — `api.ResolveVisionCapability` single resolver; `SupportsConversationalVision` deleted everywhere incl. the conversational/OCR-only distinction itself; batch splitters merged; runtime capability learning (`vision_learned.go`): image-bearing requests record accepted/rejected to `<state>/vision-capabilities.json` (30d/7d TTLs), capability rejections retry text-only so turns complete; resolver precedence runtime > probe > declared; paste/read_file/fetch_url gates route through it. Net −889 LoC through Phase 1. Phases 3–4 (delegation tier, WebUI parity) open. |
