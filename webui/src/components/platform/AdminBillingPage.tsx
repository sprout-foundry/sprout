...
const AdminBillingPage: React.FC = () => {
  ...
  const fetchCharges = async () => {
    ...
    try {
      const r = await getCharges(50);
      setCharges(r.charges);
    } catch (e) {
      debugLog('[AdminBilling] Failed to fetch charges', e);
    } finally {
  ...
  const fetchRefunds = useCallback(async () => {
    setRefunds(await getRefunds(50));
  }, []);

  const fetchDunningReport = async () => {
    const d = await getDunningReport();
    if (d) setDunningReport(d);
  };
  ...
  const handleRefundSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await processRefund({
        ...
        amount: cents,
        reason: refundReason,
        user_id: selectedCharge.user_id,
      });
      if (result) {
        ...
        <>
          ...
          <div className="platform-card" data-testid="refunds-table">
            <div className="platform-card-header">
              <h3 className="platform-card-title">Refundable Charges</h3>
            </div>
            <div className="platform-card-body">
              {loadingCharges ? (
                <p style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>Loading charges...</p>
              ) : charges.length === 0 ? (
                <p style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>No refundable charges found.</p>
              ) : (
                ...
                <div className="platform-card-body">
                  {refundSuccess ? (<div
                    ...
                    >
                      ...
                      >
                        <strong>Charge:</strong> {selectedCharge.stripe_payment_id || selectedCharge.id}
                        <br />
                        <strong>Amount:</strong> {fmtCurrency(selectedCharge.amount)}
                        <br />
                    ...
                    <tbody>
                      {refunds.map((r) => (
                        <tr key={r.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                          ...
                          <td style={{ padding: '12px' }}>
                            <span
                              className={`platform-status-badge ${r.refund_type === 'full' ? 'running' : 'warning'}`}
                            >
                          ...
                          <td style={{ padding: '12px' }}>
                            <span className="platform-status-badge running">{r.status}</span>
                          </td>
                          <td style={{ padding: '12px' }}>
                            <button
                              className="platform-button platform-button-secondary"
              ...
              <div className="platform-card" style={{ maxWidth: '480px', width: '90%' }}>
                ...
                <div className="platform-card-body">
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div>
                      <strong>Refund ID:</strong> <span data-testid="refund-id">{selectedRefund.id}</span>
                    </div>
                    <div>
                      <strong>Charge ID:</strong> <span data-testid="charge-id">{selectedRefund.charge_id}</span>
                    </div>
                    <div>
                      <strong>Amount:</strong>{' '}
                      <span data-testid="refund-amount">{fmtCurrency(selectedRefund.amount)}</span>
                    </div>
                    <div>
                      <strong>Reason:</strong> <span data-testid="refund-reason">{selectedRefund.reason}</span>
                    </div>
                    <div>
                      <strong>Status:</strong> <span data-testid="refund-status">{selectedRefund.status}</span>
                    </div>
                    <div>
                      <strong>Date:</strong> <span data-testid="refund-date">{fmtDate(selectedRefund.created_at)}</span>
                    </div>
                    {'admin_user_id' in selectedRefund && (selectedRefund as RefundWithAdmin).admin_user_id && (
                      <div>
                        <strong>Admin:</strong>{' '}
                        <span data-testid="admin-user">{(selectedRefund as RefundWithAdmin).admin_user_id}</span>
                      </div>
                    )}
                  </div>
                </div>
