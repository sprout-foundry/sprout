...
import { WorkspaceDirectory, SwitchingState, SSHFailureState, RemoteWorkspaceContext, MAX_SUGGESTIONS } from './types';
...
export function useSSHData({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!cancelled) {
        ...
        if (nextHosts.length > 0) setSelectedSshBrowseHost((c) => c && nextHosts.some((h) => h.alias === c) ? c : nextHosts[0].alias);
        else setSelectedSshBrowseHost('');
      }
    }).catch((e: unknown) => { if (!cancelled) { console.error('Failed to load SSH hosts:', e); setSshHosts([]); setSshSessions([]); } });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const next = (data.files || []).filter((f: SSHBrowseEntry) => f.type === 'directory')
        ...
        setSshSessionSuggestionsError((c) => ({ ...c, [session.key]: error instanceof Error ? error.message : 'Failed to fetch remote folders' }));
      } finally { if (!cancelled) setSshSessionSuggestionsLoading((c) => ({ ...c, [session.key]: false })); }
    })();
    return () => { cancelled = true; };
  ...
  const getSshSessionTargetPath = useCallback((s: SSHSessionEntry): string | undefined => {
  ...
  const handleOpenSshHost = useCallback(async (hostAlias: string, explicitRemotePath?: string) => {
    ...
    const statusPollTimer = window.setInterval(() => { void pollLaunchStatus(); }, 1000);
    void pollLaunchStatus();
    try {
      if (desktopBridge?.openSshWorkspace) {
        await desktopBridge.openSshWorkspace({ hostAlias, remoteWorkspacePath: targetRemotePath, forceNewWindow: true });
      } else {
        ...
        while (Date.now() < deadline) {
          try { const hr = await fetch(healthUrl, { cache: 'no-store' }); if (hr.ok) break; } catch { /* wait */ }
      ...
      if (error instanceof SSHWorkspaceOpenError) setSshFailure({ step: error.step, details: error.details, logPath: error.logPath });
      else setSshFailure(null);
      setSwitchingState({ isSwitching: false, error: message, status: null });
    } finally { statusPollCancelled = true; window.clearInterval(statusPollTimer); setIsOpeningSshHost(null); }
  ...
  const handleCloseSshSession = useCallback(async (sessionKey: string) => {
    ...
    try {
      ...
      setSwitchingState({ isSwitching: false, error: message, status: null });
    } finally { setIsClosingSshSession(null); }
  }, [setSwitchingState, setIsClosingSshSession]);

  return {
    sshHosts, sshSessions,
    ...
    setSelectedSshBrowseHost, setFocusedSshSessionKey, updateSshSessionPathDraft,
    getSshSessionTargetPath, handleOpenSshHost, handleCloseSshSession,
  };
}
