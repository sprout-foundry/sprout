...
- [x] SP-053-1d: Automated daemon shutdown test — integration_tests/test_daemon_shutdown.sh builds binary, starts daemon with isolated config, sends SIGTERM, verifies clean exit within 15s
...
## Not Started — SP-002: Provider Bug — Mistral `[TOOL_CALLS]` Format Not Parsed

Mistral-family models (e.g. `mistralai/ministral-8b-2512`, and Mistral / Codestral / Devstral variants) emit tool calls in Mistral's native `[TOOL_CALLS]` text format embedded in the assistant message **`content`**, instead of the OpenAI-style structured `tool_calls` field — observed both via OpenRouter routing and would also occur on direct Mistral. sprout has **no** handling for this anywhere (verified: `grep -rn "TOOL_CALLS" pkg/` finds only the unrelated `LOG_TOOL_CALLS` env var), so the call lands in `content` as prose, `Message.ToolCalls` stays empty, and the agentic loop treats it as a plain-text answer — **silently breaking ALL tool use for these models** (they look like they "refuse to use tools"). This affects real agent usage, not just tooling.

Discovered via the model capability probe (`pkg/modelprobe`): `ministral-8b` produced a *correct* root-cause analysis but returned it as `content="[TOOL_CALLS]submit_todos{\"summary\": \"The root cause is that user deletion only removes the user row ... without unsubscribing the user from notifications ...\"}"` with `finish_reason="stop"` and an empty `ToolCalls` slice. The model followed instructions perfectly; sprout simply never parsed the call.

- [ ] SP-002-mistralToolCalls-parse: Add a fallback in the response-normalization path so that when `Choices[i].Message.ToolCalls` is empty but `Content` contains a leading `[TOOL_CALLS]` marker, the Mistral payload is parsed into structured `api.ToolCall`s, the marker text is stripped from `Content`, and tool-call IDs are synthesized. Handle both observed shapes: `[TOOL_CALLS]<name>{<json-args>}` (one or more, possibly concatenated) and `[TOOL_CALLS][{"name":...,"arguments":{...}}, ...]` (JSON array). Non-streaming site: `pkg/agent_api/unified.go` (~L107–131, where `choice.Message` is mapped into `apiResponse.Choices[i].Message` and tool calls are converted). Gate the fallback on tools having been offered in the request, to avoid false positives on legitimate prose that happens to contain the literal string.
- [ ] SP-002-mistralToolCalls-stream: Apply the same detection on the streaming path — `pkg/agent_api/streaming.go` accumulates content deltas (`ParseSSEData` / the streaming accumulator); at finalize, if the accumulated content carries a `[TOOL_CALLS]` marker and no structured tool calls were emitted, convert it so streamed Mistral responses also work.
- [ ] SP-002-mistralToolCalls-test: Add a unit test feeding synthetic Mistral-format responses (both shapes, single and multiple calls) through normalization and asserting structured `ToolCall`s are extracted and `Content` is cleaned. Then re-run `go run ./cmd/model_probe --provider openrouter --model mistralai/ministral-8b-2512` (or `SPROUT_PROBE_TRACE=1` for per-turn detail) and confirm it now passes the complex stage rather than failing with "no todos submitted".
