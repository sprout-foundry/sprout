...
import { getServerErrorCode, isKnownServerErrorCode, dispatchServerError, type ServerErrorData } from './errorCodes';
...
describe('dispatchServerError', () => {
  it('runs the registered handler for the code', () => {
    ...
    const ran = dispatchServerError({ code: 'config_conflict', message: 'changed on disk' } as ServerErrorData, {
  ...
  it('returns false and runs nothing when no handler matches', () => {
    const handler = vi.fn();
    const ran = dispatchServerError({ code: 'unhandled_code' } as ServerErrorData, { config_conflict: handler });
  ...
  it('returns false when payload has no code at all', () => {
    const handler = vi.fn();
    const ran = dispatchServerError({ message: 'no code here' } as ServerErrorData, { config_conflict: handler });
    expect(ran).toBe(false);
    expect(handler).not.toHaveBeenCalled();
  });
