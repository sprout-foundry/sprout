...
func (lf *LogFlow) SetOutput(w io.Writer) { lf.stdout = w }

func (lf *LogFlow) out() io.Writer {
	if lf.stdout != nil {
		return lf.stdout
	}
	return os.Stdout
...
func (lf *LogFlow) showLogOptions() error {
	...
	if len(actions) == 0 {
		fmt.Fprint(lf.out(), "\r\n")
		console.GlyphInfo.Fprintf(lf.out(), "No change history available.")
		return nil
	}
...
func (lf *LogFlow) viewChangeLog() error {
	...
	console.GlyphInfo.Fprintf(lf.out(), "Recent Change History")
	...
	fmt.Fprint(lf.out(), historyText)
	fmt.Fprint(lf.out(), "\r\n")
	console.GlyphInfo.Fprintf(lf.out(), "Use /rollback <revision-id> to revert changes")
...
func (lf *LogFlow) interactiveRollback() error {
	...
	if len(revisions) == 0 {
		fmt.Fprint(lf.out(), "\r\n")
		console.GlyphInfo.Fprintf(lf.out(), "No revisions available for rollback.")
		return nil
	}
	...
	if envutil.GetEnvSimple("AGENT_CONSOLE") == "1" {
		fmt.Fprintln(lf.out())
		console.GlyphInfo.Fprintln(lf.out(), "Available Revisions:")
		...
		for i, revision := range revisions {
			...
			fmt.Fprintf(lf.out(), "%d. %s - %s (%s)\n", i+1, revision.RevisionID, description,
				revision.Timestamp.Format("2006-01-02 15:04:05"))
		}

		fmt.Fprintln(lf.out())
		console.GlyphInfo.Fprintln(lf.out(), "To rollback to a revision, use: /rollback <revision-id>")
		fmt.Fprintln(lf.out(), "   Example: /rollback rev_abc123")
		return nil
	}

	// Check if we're not in a terminal
	if !term.IsTerminal(int(os.Stdin.Fd())) {
		fmt.Fprintln(lf.out(), "Interactive rollback selection requires a terminal.")
		fmt.Fprintln(lf.out(), "Use /rollback <revision-id> to rollback to a specific revision.")
		return nil
	}
	...
	if selection < 1 || selection > len(revisions) {
		fmt.Fprintf(lf.out(), "Invalid selection. Please enter a number between 1 and %d.\n", len(revisions))
		return nil
	}
	...
	fmt.Fprintln(lf.out())
	console.GlyphWarning.Fprintf(lf.out(), "Confirm rollback of revision: %s", revisionID)
	fmt.Fprintf(lf.out(), "This will restore %d file(s). Continue? (y/n): ", len(revisions[selection-1].Changes))

	if !ui.PromptForConfirmation("") {
		fmt.Fprintln(lf.out(), "Rollback cancelled.")
		return nil
	}

	// Perform rollback
	fmt.Fprint(lf.out(), "\r\n")
	console.GlyphDim.Fprintf(lf.out(), "Rolling back to revision: %s", revisionID)
	...
	console.GlyphSuccess.Fprintf(lf.out(), "Successfully rolled back to revision: %s", revisionID)
	console.GlyphInfo.Fprintf(lf.out(), "Use '/changes' to see current session changes")
...
func (lf *LogFlow) showCurrentSessionChanges() error {
	fmt.Fprint(lf.out(), "\r\n")
	console.GlyphInfo.Fprintf(lf.out(), "Current Session Changes")

	if !lf.agent.IsChangeTrackingEnabled() {
		if lf.agent.GetChangeTracker() == nil {
			fmt.Fprintf(lf.out(), "No tracked session has started yet\r\n")
		} else {
			fmt.Fprintf(lf.out(), "Change tracking is disabled for this session\r\n")
		}
		return nil
	}
	...
	if changeCount == 0 {
		fmt.Fprintf(lf.out(), "No file changes have been tracked in this session yet\r\n")
		return nil
	}

	fmt.Fprintf(lf.out(), "Session Revision: %s\r\n", lf.agent.GetRevisionID())
	fmt.Fprintf(lf.out(), "Total Changes: %d\r\n\r\n", changeCount)

	summary := lf.agent.GetChangesSummary()
	// Convert newlines for raw mode compatibility
	summary = strings.ReplaceAll(summary, "\n", "\r\n")
	fmt.Fprint(lf.out(), summary)
...
func (lf *LogFlow) showChangeStatistics() error {
	fmt.Fprint(lf.out(), "\r\n")
	console.GlyphInfo.Fprintf(lf.out(), "Change Statistics")
	...
	if lf.agent.IsChangeTrackingEnabled() {
		...
		fmt.Fprintf(lf.out(), "Current Session:\r\n")
		fmt.Fprintf(lf.out(), "  Revision ID: %s\r\n", revisionID)
		fmt.Fprintf(lf.out(), "  Changes: %d\r\n\r\n", changeCount)
	}
...
func (lf *LogFlow) exportChangeLog() error {
	fmt.Fprint(lf.out(), "\r\n")
	console.GlyphAction.Fprintf(lf.out(), "Exporting change log...")
	...
	console.GlyphSuccess.Fprintf(lf.out(), "Change log exported to: %s", filename)
	return nil
}
