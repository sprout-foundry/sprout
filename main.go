...
type model struct {
	...
	historyIndex    int // New field to track current position in command history
}
...
func initialInteractiveModel() model {
	...
	m.commandHistory = []string{} // Ensure it's initialized
	m.historyIndex = 0            // Points to the "new command" slot initially
		...
		if m.interactiveMode && m.focusedInput {
				...
				if input != "" {
					...
					if strings.HasPrefix(input, "/") {
						...
						if handled {
							...
							m.historyIndex = len(m.commandHistory) // Reset history index to end
							if newModel != nil {
								return *newModel, cmd
							}
							return m, cmd
			...
			case "up":
				if len(m.commandHistory) > 0 {
					...
					m.textInput.SetCursor(len(m.textInput.Value())) // Move cursor to end
				}
				return m, nil
			case "down":
				if len(m.commandHistory) > 0 {
					if m.historyIndex < len(m.commandHistory)-1 {
						...
						// If at the last history item, pressing down clears the input for a new command
						m.historyIndex = len(m.commandHistory)
						m.textInput.SetValue("")
					}
				} else {
					// If no history, pressing down does nothing or clears current input if any
					m.textInput.SetValue("")
				}
				return m, nil
			case "esc":
