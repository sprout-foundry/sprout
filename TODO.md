...
### Desktop Build Dist (~727 MB)

`desktop/dist/` contains pre-built Electron release artifacts (cross-compiled backends for darwin/linux, unpacked Electron app with Chromium binaries). This is regenerated by the build pipeline and should not be checked into source control. It is not currently in .gitignore.

**Cleanup command:** `rm -rf desktop/dist/`
**Also add to .gitignore:** `desktop/dist/`
