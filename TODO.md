...
- [ ] SP-082-1: Replace `map[string]interface{}` with `*yaml.Node` (gopkg.in/yaml.v3) as the internal representation in `pkg/agent_tools/write_structured_file.go` and `pkg/agent_tools/patch_structured_file.go`. _Status: **DEFERRED**. Initial scaffolding (yaml_node_adapter.go with yamlNodeToMap/mapToYamlNode; write path through argsToYamlNode; patch path with deserializeToNode) was attempted in a 30-minute orchestrator session but hit the timeout before round-trip tests could verify correctness. Critical issue discovered during review: `mapToYamlNode` iterates Go's randomized `map[string]interface{}` order, so the patch path's final `mapToYamlNode(doc)` step re-randomizes keys — defeating the order-preservation goal. To genuinely preserve order through patch ops, `applyPatchOperation` itself must walk the `*yaml.Node` tree directly (PHASE 3 of the spec), not convert to map at the boundary. Recommend dedicating a full session to SP-082 with a coder+tester pair working iteratively on the yaml.Node-walking patch implementation, not just an adapter layer._
