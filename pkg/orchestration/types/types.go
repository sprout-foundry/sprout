...
// UnmarshalJSON implements custom JSON unmarshaling to handle detailed_guidance and patch_resolution fields
func (c *CodeReviewResult) UnmarshalJSON(data []byte) error {
	// First unmarshal into a temporary struct with raw JSON for both fields that need special handling
	type Alias CodeReviewResult
	aux := &struct {
		DetailedGuidance json.RawMessage `json:"detailed_guidance,omitempty"`
		PatchResolution  json.RawMessage `json:"patch_resolution,omitempty"`
		*Alias
	}{
		Alias: (*Alias)(c),
	}
	...
	// Handle the patch_resolution field - it could be a string or object
	if len(aux.PatchResolution) > 0 {
		// Initialize the PatchResolution if it doesn't exist
		if c.PatchResolution == nil {
			c.PatchResolution = &PatchResolution{}
		}

		// The PatchResolution.UnmarshalJSON will handle both string and object formats
		if err := c.PatchResolution.UnmarshalJSON(aux.PatchResolution); err != nil {
			return err
		}
	}
