...
| SP-132 | [WebUI API Consistency & Boilerplate Elimination](./SP-132-webui-api-consistency.md) | 🟢 Phases 1–3 shipped — `requireMethod` helper (P1, 105 sites migrated, -125 lines), `writeJSONError` consolidated (P2), bare `json.NewEncoder` migrated to `writeJSON` (P3, 165 sites, -275 lines). Phases 4–7 pending (http.Error→JSON, file splitting, locking audit, goroutine recovery). |
