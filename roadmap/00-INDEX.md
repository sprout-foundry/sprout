...
| SP-121-11 | [Git Provider OAuth + Multi-Repo](./SP-121-11-github-oauth-and-multi-repo.md) | ✅ Multi-repo workspace (repo tab bar, attach/detach/switch), unified dashboard with provider badges (🐙GitHub/🦊GitLab/🔵Bitbucket), Integrations view (provider cards, connect/disconnect, PAT fallback), `user_oauth_tokens` DB table, `GET /user/me/provider-status` endpoint. |
| SP-CLOUD | [Cloud-First WebUI](./SP-CLOUD-SPEC.md) | ✅ All 8 sub-specs verified. WASM streaming (EventBus token forwarding), Auto tier (free-tier LLM access with daily limits, DeepInfra overflow), ENV=production (usage gates, CORS, webhook validation active). |
