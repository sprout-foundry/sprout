...
var dxProvenanceHashRE = regexp.MustCompile(`source-hash: (fnv1a64:[0-9a-f]{16})`)
...
func TestDesignExportHandler_ProvenanceHeaderInEveryArtifact(t *testing.T) {
	...
	for _, f := range out.Files {
		...
		// The header must be the first line, i.e. a banner.
		assert.Regexp(t, `^(//|/\*) Generated by design_export_tokens`, string(content))
	}
}
...
func TestDesignExportHandler_ProvenanceHeaderDeterministic(t *testing.T) {
	...
	for _, name := range names {
		...
		require.Contains(t, string(data), "source-hash: fnv1a64:")
