...
| SP-130 | [Home-Directory Workspace Gate](./SP-130-home-workspace-gate.md) | 🟡 Pending — spec written 2026-07-31. Prevents the daemon from silently running in `$HOME` (root cause of macOS TCC "access Apple Music" prompts) by gating on explicit workspace selection and removing recursive home-directory inspection from the startup hot path. |
