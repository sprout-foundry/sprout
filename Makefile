...
test-unit:
	...
	go test -tags "browser" ./pkg/... ./cmd/... -v -timeout=300s -short -coverprofile=/tmp/sprout-unit-coverage.out 2>&1 | tee /tmp/sprout-test-unit.log; \
	status=$${PIPESTATUS[0]}; \
	if [ $$status -ne 0 ]; then \
		echo ""; \
...
test-coverage:
	...
	@bash -lc 'set -o pipefail; \
	go test -race -tags "browser" ./pkg/... ./cmd/... -timeout=600s -short -coverprofile=/tmp/sprout-coverage.out 2>&1 | tee /tmp/sprout-test-coverage.log; \
	status=$${PIPESTATUS[0]}; \
	if [ $$status -ne 0 ]; then \
		echo ""; \
